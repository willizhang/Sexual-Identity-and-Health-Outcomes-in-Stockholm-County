...
d_2021_complete$psychological_distress_continuous <- d_2021_complete$F21Kessler6
d_2021_complete$psychological_distress_binary <- factor( ifelse( d_2021_complete$F21Kessler6 < 8, "No", "Yes" ),
                                                         levels = c( "Yes", "No" ) ) # psychological distress defined as a score >= 8
table( d_2021_complete$psychological_distress_binary, useNA = "always" )

# suicidal thoughts
table( d_2021_complete$F21F19_Q31, useNA = "always" )
d_2021_complete$suicidal_thoughts <- factor( 
  case_when(
    d_2021_complete$F21F19_Q31 == 1 ~ "Never",
    d_2021_complete$F21F19_Q31 %in% c( 2, 3, 4 ) ~ "Ever"
  ),
  levels = c( "Never", "Ever" ) )
...
d_2021_complete$suicidal_attempts <- factor( 
  case_when(
    d_2021_complete$F21F20_Q32 == 1 ~ "Never",
    d_2021_complete$F21F20_Q32 %in% c( 2, 3, 4 ) ~ "Ever"
  ),
  levels = c( "Never", "Ever" ) )
table( d_2021_complete$suicidal_attempts, useNA = "always" )
```
...
results_list_by_country_of_birth <- calculate_svyby_proportion_subgroup( 
  ...
  design = survey_design_cc,
  year = 2021,
  group_var = "sexual_identity_2021 + country_of_birth" )

self_report_health_outcomes_2021_by_country_of_birth <- Reduce( function( x, y ){
  merge( x, y, by = c( "sexual_identity_2021", "country_of_birth" ) )
}, 
results_list_by_country_of_birth )
writexl::write_xlsx( self_report_health_outcomes_2021_by_country_of_birth, "prevalence_cc_self_report_health_outcomes_2021_by_country_of_birth.xlsx" )
```
