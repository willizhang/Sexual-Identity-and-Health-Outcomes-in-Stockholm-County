...
# self-rated health
table( d_2010_complete$U03G03, useNA = "always" )
d_2010_complete$U03G03[ d_2010_complete$U03G03 == 9 ] <- NA
d_2010_complete$poor_health <- factor( if_else( d_2010_complete$U03G03 >= 4, "Yes", "No" ),
                                       levels = c( "Yes", "No" ) )
table( d_2010_complete$poor_health, useNA = "always" )

# chronic health condition or disability
table( d_2010_complete$U04aG04a, useNA = "always" )
d_2010_complete$U04aG04a[ d_2010_complete$U04aG04a == 9 ] <- NA
d_2010_complete$chronic_condition <- factor( if_else( d_2010_complete$U04aG04a == 2, "Yes", "No" ),
                                             levels = c( "Yes", "No" ) )
table( d_2010_complete$chronic_condition, useNA = "always" )
```
...
# frequent heavy episodic drinking (at least once per week)
table( d_2010_complete$U34G38, useNA = "always" )
d_2010_complete$U34G38[ d_2010_complete$U34G38 == 9 ] <- NA
table( d_2010_complete$U32G36, useNA = "always" )
d_2010_complete$U32G36[ d_2010_complete$U32G36 == 9 ] <- NA
d_2010_complete$U34G38 <- ifelse( d_2010_complete$U32G36 == 1 & is.na( d_2010_complete$U34G38 ), 7, d_2010_complete$U34G38 )
d_2010_complete$frequent_HED <- factor( if_else( d_2010_complete$U34G38 <= 3, "Yes", "No" ),
                                        levels = c( "Yes", "No" ) )
table( d_2010_complete$frequent_HED, useNA = "always" )
...
variables_list <- list(
  list( variable = "poor_health", condition = "Yes", name = "poor_health" ),
  list( variable = "chronic_condition", condition = "Yes", name = "chronic_condition" ),
  ...
  list( variable = "frequent_HED", condition = "Yes", name = "frequent_HED" ),
...
```{r}
d_pdr_regitry <- read_sas("/Volumes/LGBT Project data/Registry-based health outcomes/atc_2002_2014.sas7bdat")
summary( d_pdr_regitry )
unique( d_pdr_regitry$ATC )

d_pdr_regitry <- d_pdr_regitry %>%
  filter( ATC != "N05CD" ) # include participants who HAD at least one prescription record of N05B or N06A from 2005-06-30 to 2021-12-31

unique( d_pdr_regitry$ATC )
summary( d_pdr_regitry ) # 306 rows have missing values in DDD
nrow( d_pdr_regitry[ d_pdr_regitry$ddd == 0 & !is.na( d_pdr_regitry$ddd ), ] ) # 30 rows have a DDD value of 0
View( d_pdr_regitry[ d_pdr_regitry$ddd == 0 | is.na( d_pdr_regitry$ddd ), ] )
d_pdr_regitry$ddd <- ifelse( d_pdr_regitry$ddd == 0, NA, d_pdr_regitry$ddd )
...
d_pdr_regitry <- d_pdr_regitry %>%
  mutate(
    ...
    ddd_N06A = if_else( presence_N06A == "Yes", ddd, 0 ),
    ddd_N05B = if_else( presence_N05B == "Yes", ddd, 0 ),
    year = year( EDATUM )
    )

summary( d_pdr_regitry )
sum( d_pdr_regitry$ddd, na.rm = TRUE )
...
summary( count_ATC )
miss_var_summary( count_ATC ) # 30 missing in ddd_N06A and 7 missing in ddd_N05B
...
ggplot( 
  ...
  labs(
    x = "Calendar Year",
    y = "Average Number of Dispensed Prescriptions"
  ) +
