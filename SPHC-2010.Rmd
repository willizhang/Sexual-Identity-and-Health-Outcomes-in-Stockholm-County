...
d_pdr_regitry <- read_sas("/Volumes/LGBT Project data/Mental health outcomes/atc_2002_2014.sas7bdat") %>%
  filter( ATC != "N05CD" )

d_pdr_regitry_wide <- d_pdr_regitry %>%
  group_by( lopnr ) %>%
  mutate( row = row_number() ) %>%
  pivot_wider( names_from = row, values_from = c( ATC, EDATUM ) )

d_pdr_regitry_cleaned <- d_pdr_regitry_wide %>%
  rowwise() %>%
  mutate(
    
    # first-time use since 2005
    first_use_anxiolytics = {
      dates <- c_across( starts_with( "EDATUM_" ) )[ grepl( "N05B", c_across( starts_with( "ATC_" ) ) ) ]
      if ( length( dates ) > 0 ) min( dates, na.rm = TRUE ) else NA
      },
    first_use_antidepressants = {
      dates <- c_across( starts_with( "EDATUM_" ) )[ grepl( "N06A", c_across( starts_with( "ATC_" ) ) ) ]
      if ( length( dates ) > 0 ) min( dates, na.rm = TRUE ) else NA
      },
    
    # any use since 2005
    any_use_anxiolytics = if_else(any(grepl("N05B", c_across(starts_with("ATC_")))), "Yes", "No"),
    any_use_antidepressants = if_else(any(grepl("N06A", c_across(starts_with("ATC_")))), "Yes", "No")
    ) %>%
  ungroup()

# count no. of prescriptions per year
for ( year in 2005:2021 ) {
  d_pdr_regitry_cleaned <- d_pdr_regitry_cleaned %>%
    rowwise() %>%
    mutate(
      !!paste0( "anxiolytics_", year ) := count_prescriptions_per_year(
        c_across( starts_with( "ATC_" ) ), 
        c_across( starts_with( "EDATUM_" ) ), 
        year, 
        "N05B"
        ),
      !!paste0( "antidepressants_", year ) := count_prescriptions_per_year(
        c_across( starts_with( "ATC_" ) ), 
        c_across( starts_with( "EDATUM_" ) ), 
        year, 
        "N06A"
...
d_pdr_regitry_final <- d_pdr_regitry_cleaned %>%
  select( lopnr, matches( "antidepressants|anxiolytics" ) )

d_2010_complete_merged <- d_2010_complete %>%
  left_join( d_pdr_regitry_final, by = "lopnr" )
