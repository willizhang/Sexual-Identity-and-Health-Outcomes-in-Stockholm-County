  ...
  mutate( 
    drug_class = as.factor( 
      case_when(
        ...
        grepl( "^N05B", ATC ) ~ "N05B" ) ),
      year = year( EDATUM ) ) %>%
  filter( year == 2010 ) # select prescription data only in the year 2010
...
atc_2010_summary <- atc_2010 %>%
  ...
  summarise( prescription_count_2010 = n(), .groups = "drop" )

atc_2010_summary_wide <- atc_2010_summary %>%
  pivot_wider(
    names_from = drug_class,
    values_from = prescription_count_2010,
    names_prefix = "prescription_count_2010_",
    values_fill = 0
    ) %>%
  mutate(
    prescribed_2010_N06A = factor( if_else( prescription_count_2010_N06A > 0, "Yes", "No" ) ),
    prescribed_2010_N05B = factor( if_else( prescription_count_2010_N05B > 0, "Yes", "No" ) ) )
summary( atc_2010_summary_wide )

d_2010_complete_cc_linked <- d_2010_complete %>%
  left_join( atc_2010_summary_wide, by = "lopnr" ) %>%
  mutate(
    prescription_count_2010_N06A = if_else( !is.na( lopnr ) & is.na( prescription_count_2010_N06A ), 0, prescription_count_2010_N06A ),
    prescription_count_2010_N05B = if_else( !is.na( lopnr ) & is.na( prescription_count_2010_N05B ), 0, prescription_count_2010_N05B ),
    prescribed_2010_N06A = factor( if_else( !is.na( lopnr ) & is.na( prescribed_2010_N06A ), "No", prescribed_2010_N06A ) ),
    prescribed_2010_N05B = factor( if_else( !is.na( lopnr ) & is.na( prescribed_2010_N05B ), "No", prescribed_2010_N05B ) )
    )

# cross check data
summary( d_2010_complete_cc_linked$prescription_count_2010_N06A )
sum( d_2010_complete_cc_linked$prescription_count_2010_N06A, na.rm = TRUE )
sum( atc_2010_summary_wide$prescription_count_2010_N06A )
summary( d_2010_complete_cc_linked$prescription_count_2010_N05B )
sum( d_2010_complete_cc_linked$prescription_count_2010_N05B, na.rm = TRUE )
sum( atc_2010_summary_wide$prescription_count_2010_N05B )
table( d_2010_complete_cc_linked$prescribed_2010_N06A, useNA = "always" )
table( atc_2010_summary_wide$prescribed_2010_N06A, useNA = "always" )
table( d_2010_complete_cc_linked$prescribed_2010_N05B, useNA = "always" )
table( atc_2010_summary_wide$prescribed_2010_N05B, useNA = "always" )

# assume complete follow-up through registry during 2010 among SPHC-B 2010 participants
...
survey_design_cc <- svydesign( ids = ~ 1, 
                               ...
                               fpc = ~ no.of.population,
                               data = d_2010_complete_cc_linked[ !is.na( d_2010_complete_cc_linked$lopnr ), ] )
```
...
pdr_2010_overall <- Reduce( function( x, y ){
  cbind( x, y )
}, 
results_list_overall ) %>% 
  as.data.frame() %>%
  rownames_to_column( var = "sexual_identity_2010" )
pdr_2010_overall[ 1, 1 ] <- "Stockholm County"

writexl::write_xlsx( rbind( pdr_2010, pdr_2010_overall ), "prevalence_pdr_2010.xlsx" )
...
pdr_2010_by_sex <- Reduce( function( x, y ){
  merge( x, y, by = c( "sexual_identity_2010", "sex" ) )
}, 
results_list_by_sex )
writexl::write_xlsx( pdr_2010_by_sex, "prevalence_pdr_2010_by_sex.xlsx" )
...
pdr_2010_by_age <- Reduce( function( x, y ){
  merge( x, y, by = c( "sexual_identity_2010", "age_cat" ) )
}, 
results_list_by_age )
writexl::write_xlsx( pdr_2010_by_age, "prevalence_pdr_2010_by_age.xlsx" )
```

##### 4.1.3. Calculate number of dispensed prescriptions
```{r}
variables_list <- c( "prescription_count_2010_N06A", "prescription_count_2010_N05B" )
...
writexl::write_xlsx( rbind( pdr_2010, pdr_2010_overall ), "count_pdr_2010.xlsx" )
...
results_list_by_sex )
writexl::write_xlsx( pdr_2010_by_sex, "count_pdr_2010_by_sex.xlsx" )
...
results_list_by_age )
writexl::write_xlsx( pdr_2010_by_age, "count_pdr_2010_by_age.xlsx" )
```
