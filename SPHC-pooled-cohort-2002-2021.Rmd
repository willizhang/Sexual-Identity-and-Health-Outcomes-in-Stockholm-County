...
d_2002$age_2021 <- d_2002$F2alder + 19
...
d_2006$age_2021 <- d_2006$F6alder + 15
...
d_2010$age_2021 <- d_2010$F10alder + 11
...
d_2014$age_2021 <- d_2014$F14alder + 7
...
d_2021$age_2021 <- d_2021$F21alder
...
#### 2.6. Merge data across surveys
```{r}
variable_list <- c( "lopnr", "sexual_identity_2010", "sexual_identity_2014", "sexual_identity_2021", "age_2021", "sex", "country_of_birth" )
...
d_pooled_cohort <- d_pooled_cohort %>% filter( !is.na( lopnr) ) # 7,411 in SPHC-B 2002 did not participate in follow-up surveys, and therefore excluded
...
duplicates <- d_pooled_cohort %>%
  filter(
    survey_year %in% c( "SPHC 2002", "SPHC 2006", "SPHC 2010", "SPHC 2014" ),
    !( survey_year == "SPHC 2002" & is.na( lopnr ) )
    ) %>%
  group_by( lopnr ) %>%
  filter( n() > 1 ) %>%
  ungroup()

duplicates$survey_year <- as.factor( duplicates$survey_year )
summary( duplicates )
n_distinct( duplicates$lopnr ) # in total, 1,140 duplicate participants across SPHC 2002, 2006, and 2014

d_pooled_cohort_cleaned <- d_pooled_cohort %>%
  group_by( lopnr ) %>%
  filter(
    !( survey_year == "SPHC 2002" & any( survey_year == "SPHC 2006" ) ), 
    !( survey_year == "SPHC 2002" & any( survey_year == "SPHC 2014" ) ),
    !( survey_year == "SPHC 2006" & any( survey_year == "SPHC 2014" ) ) # exclude duplicate participants
    ) %>%
  ungroup()

d_pooled_cohort_cleaned %>%
  filter(
    survey_year %in% c( "SPHC 2002", "SPHC 2006", "SPHC 2010", "SPHC 2014" ),
    !( survey_year == "SPHC 2002" & is.na( lopnr ) )
    ) %>%
  group_by( lopnr ) %>%
  filter( n() > 1 ) # confirm no duplicate participants

d_pooled_cohort_cleaned$survey_year <- as.factor( d_pooled_cohort_cleaned$survey_year )
summary( d_pooled_cohort_cleaned )


# generation
d_2021$birth_year <- 2021 - d_2021$age_baseline
d_2021 <- d_2021 %>%
